...
| `make test-validate-research-output` | Run the regression harness for `scripts/validate-research-output.sh` (the substantive-content validator invoked by `collect-reviewer-results.sh --substantive-validation` — Phase 3 of umbrella #413, closes #416). Exercises `scripts/test-validate-research-output.sh` — 24 cases covering the word-count threshold, fence-stripping word count, file/URL/code-fence/Makefile/leading-dot citation probes, `--no-require-citations`, the `--validation-mode` preset (NO_ISSUES_FOUND short-circuit + 30-word floor), and error paths. A `make lint` prerequisite via `test-harnesses`. |
