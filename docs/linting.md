...
| `make test-classify-research-scale` | Run the offline regression harness for `skills/research/scripts/classify-research-scale.sh` (the deterministic shell classifier invoked by `/research` Step 0.5 — closes #513). Exercises `skills/research/scripts/test-classify-research-scale.sh` — 26 cases covering the three-stage rule set with asymmetric conservatism (Stage 1 strong-deep signals: length / keyword / multi-`?`; Stage 2 strong-quick signals requiring the conjunction of length<80 + single-`?` + lookup-keyword + zero-deep-keywords; Stage 3 default-standard), failure modes (`empty_input` / `bad_path` / `missing_arg`), and asymmetric-conservatism boundaries (borderline cases default to standard, never quick). A `make lint` prerequisite via `test-harnesses`. |
...
| `make test-umbrella-parse-args` | Run the regression harness for `.claude/skills/umbrella/scripts/parse-args.sh` (closes #572). Exercises `.claude/skills/umbrella/scripts/test-umbrella-parse-args.sh` — 22 cases pinning the stdout grammar (`LABELS_COUNT` + indexed `LABEL_<i>`, `TITLE_PREFIX`, `REPO`, `CLOSED_WINDOW_DAYS`, `DRY_RUN`, `GO`, `DEBUG`, `TASK`, `UMBRELLA_TMPDIR`), the frozen `ERROR=` template list (10 templates), the quoting subset (single-quote, double-quote with `\"`/`\\`/`\$` escapes, outside-quote backslash escapes, space/tab/newline as unquoted separators), and the TASK byte-preservation contract (verbatim remainder including unbalanced quotes inside TASK). Distinct from `test-parse-args` which covers `skills/create-skill/scripts/parse-args.sh`. A `make lint` prerequisite via `test-harnesses`. |
