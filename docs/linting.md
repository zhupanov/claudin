...
| `make test-quick-mode-docs-sync` | Run the regression harness for `/implement --quick` public-docs sync (closes #370). Exercises `scripts/test-quick-mode-docs-sync.sh` in both default mode (positive-anchor + stale-phrase checks against `README.md`, `docs/review-agents.md`, `docs/workflow-lifecycle.md`, and `skills/implement/SKILL.md`) and `--self-test` mode (embedded good/bad fixtures proving the check mechanics). Canonical source of truth for the enforced markers and stale-phrase list is the script itself + sibling `scripts/test-quick-mode-docs-sync.md`. A `make lint` prerequisite. |
