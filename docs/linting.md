...
| `make test-validate-research-output` | Run the regression harness for `scripts/validate-research-output.sh` (the substantive-content validator invoked by `collect-reviewer-results.sh --substantive-validation` — Phase 3 of umbrella #413, closes #416, #447). Exercises `scripts/test-validate-research-output.sh` — 35 cases covering the word-count threshold, fence-stripping word count, file/URL/code-fence/Makefile/leading-dot citation probes (probe 1 with the #447-broadened 51-extension list and trailing-token boundary that rejects fake citations like `file.mdjunk:42`), `--no-require-citations`, the `--validation-mode` preset (NO_ISSUES_FOUND short-circuit + 30-word floor), and error paths. A `make lint` prerequisite via `test-harnesses`. |
