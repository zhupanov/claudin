...
External agents in `/research` and `/review` launch directly against the working tree (`cursor agent ... --workspace "$PWD"`, `codex exec --full-auto -C "$PWD"`) and inherit the user's filesystem privileges. For review and research tasks, the prompt asks them not to modify files — this is a behavioral constraint, not a sandbox. `/implement` Step 2 implementation is delegated to Codex (`codex exec --full-auto`) via the dispatcher `skills/implement/scripts/step2-implement.sh`. The spawn is **mandatory when Codex is available** — the dispatcher is the single branch on `codex_available`, and only when Step 0's standard `CODEX_AVAILABLE && CODEX_HEALTHY` test reports unavailable does the dispatcher emit `STATUS=claude_fallback` and route the orchestrator to the Edit/Write code-edit path. Codex returns a JSON manifest (schema: `skills/implement/references/codex-manifest-schema.md`) which the dispatcher validates mechanically (path normalization, baseline-rooted `git diff` set-equality, branch / `.claude-plugin/plugin.json` / submodule unchanged checks) and sanitizes (`scripts/redact-secrets.sh` over commit message, summary bullets, todos, OOS observations) before downstream Steps 4 / 8a / 9a / 9a.1 consume it. Codex's full transcript stays on disk; Claude does NOT inspect it or run `git diff` to figure out what changed. On `STATUS=needs_qa`, Codex writes a `qa-pending.json` companion file (under the dispatcher-managed tmpdir; the dispatcher validates its presence and shape before surfacing `STATUS=needs_qa`); the orchestrator collects answers (`AskUserQuestion` when `auto_mode=false`) and re-invokes the dispatcher with `--answers` — Codex resumes from its prior partial work on the branch rather than restarting. Resume cycles are capped at 5 (6th `--answers` invocation auto-bails with `qa-loop-exceeded`). See [`SECURITY.md` § External tool delegation](../SECURITY.md) for the full enumeration of post-Codex mechanical checks. Cursor participates only as a reviewer, sketch author, and voter — it does not modify the working tree during `/implement`. The `/research` skill carries a skill-scoped `PreToolUse` hook (`scripts/deny-edit-write.sh`) that mechanically guards Claude's own `Edit | Write | NotebookEdit` tool surface to canonical `/tmp` only; the hook does **not** cover Bash or subprocess-spawned external agents. See [`SECURITY.md` § External reviewer write surface in /research](../SECURITY.md#external-reviewer-write-surface-in-research) for the full trust-model framing and [`docs/review-agents.md` § External reviewer trust boundary](review-agents.md#external-reviewer-trust-boundary-skills-using-cursor--codex-against-pwd) for the skill-author-facing summary.
