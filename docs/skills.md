...
Process one approved GitHub issue per invocation. Fetches open issues with a `GO` sentinel comment, skips any blocked by an open dependency (GitHub's native blocked-by API plus conservative prose-keyword scanning of the body and comments — `Depends on #N`, `Blocked by #N`, etc., with fail-open posture), triages, and classifies intent (PR/NON_PR) and — for PR tasks — complexity (SIMPLE/HARD). PR tasks delegate to `/implement` with `--issue $ISSUE_NUMBER` forwarded so the queue issue is adopted as the tracking issue (no separate tracking issue is created); NON_PR tasks run inline (typically filing findings via `/issue`) and never call `/implement`. With a number or URL argument, targets a specific issue instead of auto-picking. Single-iteration design — the caller handles repetition. `--no-slack` is forwarded to the delegated `/implement` run (suppressing its Step 16a issue Slack post) AND suppresses `/fix-issue`'s own Step 8 NON_PR-path Slack announcement. Default (no `--no-slack`): both paths post per `/implement`'s default-on behavior (gated on Slack env vars).
...
**Optional `--plan` (planner pre-pass — standard scale only)**: when set, a single Claude Agent subagent decomposes `RESEARCH_QUESTION` into 2–4 focused subquestions before the lane fan-out. Each lane researches its assigned subquestion(s) (deterministic assignment: N=2 → all lanes get the union; N=3 → one per lane; N=4 → lane #1 (Cursor) gets two, lane #2 (Codex) and lane #3 (Claude inline) get one each). Synthesis is organized by subquestion sub-section + a final cross-cutting findings sub-section. Falls back cleanly to single-question mode on any planner failure (count out of [2,4], empty output, prose-only reply, timeout) with a visible warning. Default off — byte-equivalent to pre-#420 behavior. **Not applicable to `--scale=quick`** (single lane → no decomposition benefit) or `--scale=deep` (deep mode's 4 named angle prompts already differentiate per-lane focus; combining with subquestion focus is documented as future work). Both incompatible combinations downgrade `--plan` to off with a visible warning at the start of Step 1.
