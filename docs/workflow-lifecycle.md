...
| `--scale=quick\|standard\|deep` | `/research` | Selects the lane shape for the research and validation phases. Default `standard` (3+3 lanes; byte-equivalent to pre-#418 behavior). `quick` runs 1 inline Claude lane and skips Step 2 (validation panel) — the report still renders a 0-reviewer Validation phase placeholder line (fastest, single-lane confidence — explicit lower assurance). `deep` runs 5+5 lanes with diversified angle prompts (architecture / edge cases / external comparisons / security) and 2 extra Claude code-reviewer subagents in validation. Independent of `--debug`. |
