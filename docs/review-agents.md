...
External reviewers (Codex, Cursor) produce single-list output — their entire output is treated as in-scope findings. (`/research` validation also keeps a single-list contract: even though `scripts/render-reviewer-prompt.sh` emits the dual-list-shaped archetype, the rendered prompt instructs models to leave the Out-of-Scope Observations section empty for research validation, preserving the negotiation pipeline's single-list invariant.)
...
**Note A**: `/research` uses a 3-lane composition under the Negotiation Protocol; `/loop-review` post-overhaul delegates to per-slice `/review --slice-file` subprocesses, each running the standard 3-lane Voting Protocol panel. Both protocols use the same 3-lane shape, matching the `/design` and `/review` topology. Lane count is independent of protocol choice. Every full 3-reviewer panel in this repo shares the same 3-attribution shape (`Code`, `Codex`, `Cursor`), with a single Claude Code Reviewer subagent fallback per unavailable external preserving the 3-lane invariant. Exceptions: `/implement` quick mode runs a single-reviewer loop of up to 7 rounds with a per-round Cursor → Codex → Claude Code Reviewer subagent fallback chain (no voting panel), and voting panels may collapse to 2 or skip per the threshold rules in `skills/shared/voting-protocol.md`.
