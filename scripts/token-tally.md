...
- `--scale` drives label rendering and validation-row branching. Pass `quick` when `RESEARCH_QUICK=true` (the post-#520 `/research --quick` path: K=3 homogeneous Claude inline subagents, all measurable; Step 2 validation is skipped). Pass `standard` or `deep` for the multi-tool research/validation modes. Callers (e.g., `skills/research/SKILL.md` Step 4) MUST derive an effective `TALLY_SCALE` per the runtime mode rather than passing `RESEARCH_SCALE` raw — `--quick` is orthogonal to `RESEARCH_SCALE` (which retains its default `standard` value), so passing `RESEARCH_SCALE` raw under `--quick` would render quick-mode telemetry as standard-mode.
