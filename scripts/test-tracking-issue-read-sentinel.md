...
## Test cases (14 total, 28 assertions)
...
| h  | `ADOPTED=true` + trailing space    | 1             | envelope names the trailing-space value (quoted `'true '` in the error)                                      |
...
| l  | CRLF line endings                  | 0             | `\r` stripped: `ADOPTED=true`                                                                                |
| m  | UTF-8 BOM-prefixed file            | 0             | BOM stripped: first key on first line parses correctly                                                       |
| n  | leading whitespace (`␠␠ADOPTED=…`)| 0             | column-0 rule: line unmatched, emits `ADOPTED=`                                                              |

Happy-path cases (a, b, c, d, j, k, l, m, n) use `assert_equal_stdout` against the full expected stdout string to pin exact 3-line shape and ordering. Failure cases (e, f, g, h, i) assert both exit code and the exact envelope (for (e) and (i)) or at least the quoted rejected value in the ERROR line (for (f), (g), (h)).
