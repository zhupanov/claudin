...
**Purpose**: Test harness for `scripts/render-specialist-prompt.sh`. Validates that all specialist agent files exist, have valid frontmatter, and render correctly in both diff and slice modes. Also validates error handling for invalid inputs.
...
4. Slice mode rendering (slice preamble, canonical file list, OOS anchor)
5. Competition notice flag (absent without flag, present with flag)
6. Error cases (missing args, invalid mode, nonexistent file, incomplete slice args)
