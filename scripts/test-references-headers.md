...
**Contract-field line-range rejection**. For each `skills/*/references/*.md`, the harness extracts the Contract paragraph — lines from the anchored `^\*\*Contract\*\*:` header through (and including) the next blank line — and rejects any citation matching the ERE pattern `L[0-9]+[-–][0-9]+`. Both ASCII hyphen (`-`) and en-dash (`–`) are matched because GitHub Markdown rendering occasionally substitutes the latter. **Why**: the four `skills/implement/references/*.md` files historically carried stale source-of-truth line-range citations (`L522-528`, `L758-898`, `L1030-1123`, `L580-681`) that drifted silently as SKILL.md was edited later; v5.2.7 replaced all four with range-free descriptions (FINDING_4), and issue #322 was the OOS follow-up tracking the need for a regression guard so no future reference file can reintroduce the drift pattern. **Edit-sync rule**: when describing a Contract, cite the authoritative concept (e.g., "Phase 1 classification", "Step 2.4 collection", "the `--branch-info` 4-key requirement") rather than source-line ranges; if a future maintainer genuinely needs to cite a fixed range, update this harness to exclude that file and document the exemption.
...
- Do NOT reintroduce `L\d+-\d+` line-range citations inside a Contract paragraph; use range-free conceptual descriptions. If a bona-fide exemption is needed, add the exempt path to the harness's Contract-field scan rule and document the reason here.
