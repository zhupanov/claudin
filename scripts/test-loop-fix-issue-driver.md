...
| G | `invoke_claude_p_skill` preserves FINDING_7 (`--plugin-dir`), FINDING_9 (STDIN delivery), FINDING_10 (stderr sidecar) contracts AND pins `--output-format stream-json` and `--verbose` (NDJSON capture so the Step 0 success breadcrumb reaches `iter-N-out.txt`; default-mode `claude -p` would emit only the final assistant message text) |
...
- The stream-json contract pins (`--output-format stream-json`, `--verbose`) are byte-pinned via independent fixed-string greps; the order of flags in the live argv may evolve, but both substrings must remain present in the `invoke_claude_p_skill` body. Removing either flag would silently regress the loop to default-mode `claude -p` (where intermediate-turn breadcrumbs are dropped) and reintroduce the Step 0 sentinel-mismatch halt class.
