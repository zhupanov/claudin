...
- **stderr/stdout discipline**: all diagnostics and progress messages on stderr; ONLY the rendered prompt body on stdout (via `printf '%s\n'` semantics, terminated by the trailing newline of the last line). The rendered output is consumed verbatim by `cursor-wrap-prompt.sh` and `codex exec`; mixing diagnostics into stdout would corrupt the prompt.
