...
#   B) SKILL.md body declares LOOP_DRIVER_LOG_FILE env-overridable default
#      AND the /tmp/+/private/tmp/ case-arm validation (security boundary).
#   C) SKILL.md body surfaces the log path: at least one '📄 Full driver log:'
#      pre-launch line AND at least one '📄 Full driver log (retained):' completion line.
...
#
# Exit 0 on all-pass; exit 1 otherwise. Wired into make lint via test-loop-review target.

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"
SKILL_MD="$REPO_ROOT/skills/loop-review/SKILL.md"
...
# --- Assertion B: LOOP_DRIVER_LOG_FILE + /tmp validation ---
if ! grep -qF 'LOOP_DRIVER_LOG_FILE' "$SKILL_MD"; then
  fail "B: SKILL.md missing LOOP_DRIVER_LOG_FILE env-overridable default"
else
