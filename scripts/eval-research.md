...
| `--scale <s>` | `standard` | Forwarded to `/larch:research` as `--scale=<s>`, manually overriding the adaptive scale classifier (issue #513) so the harness deterministically tests the labeled scale. The chosen scale is recorded in `eval-baseline.json`'s top-level `scale` field for cross-run baseline comparability. |
