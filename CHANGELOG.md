...
- **BREAKING**: `/research --scale=quick` is removed. The legacy single-lane `quick` mode (1 Claude inline lane, no validation panel, "single-lane confidence" disclaimer) is replaced by a new orthogonal `--quick` boolean flag that runs `RESEARCH_QUICK_K=3` (hardcoded) homogeneous Claude inline lanes carrying identical `RESEARCH_PROMPT_BASELINE` plus a structured-output rubric (each lane emits a `<lane_claims>` block with claims/evidence/unknowns), then vote-merges via reproducible Jaccard-based clustering (≥0.6 Jaccard over normalized claim keys). `--scale`'s value enum is reduced to `standard|deep` (default `standard`); legacy callers passing `--scale=quick` hit a fatal error with a migration pointer to `--quick`. `--quick` is mutually exclusive with explicit `--scale=standard|deep`; the implicit `standard` default does NOT conflict with `--quick`. Three-tier partial-failure handling: 3/3 lanes succeed → full vote-merge; 2/3 → degraded vote-merge with explicit "1 lane failed" disclaimer prepended to synthesis; <2 (0/3 or 1/3) → fail-closed (abort run). An internal `quick-ballot.md` artifact (per-lane parsed claims + cluster mapping) is written to `$RESEARCH_TMPDIR` and preserved alongside the findings sidecar at Step 4 when `KEEP_SIDECAR=true` (using the same destination-validation guardrails as the findings sidecar `cp`). `--plan + --quick` is downgraded with a warning citing the homogeneous-lane requirement (K=3 vote-merge requires identical `RESEARCH_PROMPT_BASELINE` across all K lanes for the consensus comparison; subquestion decomposition would split lanes across different questions, defeating vote-merge). `skills/research/data/quick-disclaimer.txt` is rewritten from the single-line "Single-lane confidence" form to a multi-line "K-lane vote-merge confidence" disclaimer that explicitly names correlated-error risk: vote-merge across K=3 homogeneous Claude lanes is **not validation** (Step 2 was skipped) and **not cross-tool diversity** (all 3 lanes share the same model — shared training data, shared inductive biases may reproduce the same wrong answer K times). Closes #520.
- Co-evolved with /design + /review: 1 design round (5-agent sketch all converged on the orthogonal `--quick` boolean + `RESEARCH_QUICK` mental flag + `SCALE_EXPLICIT` tracker; 3-decision dialectic resolved D1 ANTI_THESIS 2-1 (structured per-lane `<lane_claims>` + Jaccard clustering, NOT pure prose rubric), D2 THESIS 3-0 (three-tier partial-failure handling), D3 ANTI_THESIS 2-1 (internal `quick-ballot.md` artifact preserved via `--keep-sidecar`); user Round 2 confirmed all dialectic outcomes; 3-reviewer plan-review with 10 of 11 in-scope findings accepted including FINDING_1 (Check 11/12 numbering correction + invalid-scale literal), FINDING_2 (flag-independence + Check 13), FINDING_3 (validation-phase.md sweep), FINDING_4 (test-render-findings-batch Case 10), FINDING_5 (clustering-determinism spec), FINDING_6 (docs/skills.md explicit), FINDING_7 (ballot cp guardrails), FINDING_8 (SECURITY.md update), FINDING_10 (--adjudicate+--quick message), FINDING_11 (RESEARCH_SCALE!=quick gate explicit); 1 unanimously exonerated (F9 — extracted helper + offline harness); 2 OOS accepted for filing (substantive validation gap for K=3, /research docs-sync harness)) + 1 code review round (3 in-scope findings, 2 accepted including F1 `token-tally.sh report --scale` overload under `--quick` and F3 `--tokens` vs `--total-tokens` flag mismatch in quick-lane sidecar; F2 `subagent_type=general-purpose` deviation rejected as consistency preference per 0Y/1E/2N panel vote).
- `SECURITY.md` extended with a new `/research --quick` lower-assurance mode bullet (#418, #520) replacing the legacy `--scale=quick` documentation: explicit "K-lane vote-merge confidence" disclaimer, correlated-error risk framing, three-tier partial-failure handling, and a quick-mode ballot write surface description (when `--keep-sidecar` is set with `--quick`, an additional `<base>-ballot.md` artifact is also written to disk alongside the findings sidecar, using the same destination-validation guardrails). Synthesis + revision subagent entry (#507) updated to clarify that quick mode (now `--quick`, `RESEARCH_QUICK=true`) is unaffected by these subagents — vote-merge synthesis remains orchestrator-inline (the K=3 lane subagents are a separate pattern). Planner subagent entry (#420 + #519 + #520) re-keys the `--plan` downgrade trigger from `--scale=quick` to `--quick`.
- Public-doc mirrors updated: `README.md` flag table, `docs/skills.md` `/research` section, `docs/workflow-lifecycle.md` `/research` bullet + flag table, `docs/agents.md` Code Reviewer + Research Agents sections, `docs/external-reviewers.md` lane-count summary, `docs/review-agents.md` panel composition table all updated to reflect the orthogonal `--quick` boolean and the legacy-fatal `--scale=quick`. `scripts/token-tally.sh` `report` command's quick branch updated from "1 lane — Claude inline only" to "3 lanes — K=3 homogeneous Claude inline, all measurable" and the "skipped in --scale=quick" message re-keyed to "skipped in --quick"; the no-measurements catch-all message branches per scale to distinguish quick (K=3 measurable lanes that should have written sidecars) from standard/deep (Claude inline + unmeasurable externals). `scripts/test-research-structure.sh` adds Checks 9b and 12b for the new `--quick` flag literal and the legacy `--scale=quick` migration-pointer error literal, and re-keys Checks 9, 11, 12, and 21e for the renamed Quick subsection (`### Quick (RESEARCH_QUICK=true)` + `K-lane vote-merge confidence` disclaimer). `skills/research/scripts/test-render-findings-batch.sh` Case 10 hardcoded disclaimer literal updated to match the new K-lane prose. Closes #520.
