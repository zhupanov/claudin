...
Parser regression coverage lives in `${CLAUDE_PLUGIN_ROOT}/skills/issue/scripts/test-parse-input.sh` (self-contained; run manually via `bash skills/issue/scripts/test-parse-input.sh` — not wired into automated CI).
