...
**Coverage** — Bash 3.2 portability (issue #744):

- **Static portability guard**: `grep -nE 'declare -A|mapfile|\$\{[A-Z_]+,,\}' helpers.sh | grep -vE '^[0-9]+:[[:space:]]*#'` returns empty. Mirrors `test-allocate-candidates.sh` Test 21 including the comment-line filter so explanatory comments mentioning the banned constructs do not false-positive CI. Runs on every harness invocation; CI catches future regressions even on Linux Bash 5.

**Coverage** — `wire-dag` Bash 3.2-safe cache regressions (issue #744):
