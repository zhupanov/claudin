...
**Test coverage**: the malformed-`pieces.json` gatekeeper contract is pinned by `test-render-batch-input.sh` (sibling `test-render-batch-input.md`), wired into `make lint` via the `test-umbrella-render-batch-input` Makefile target. The harness pins FIVE failure modes at the script boundary: (a) JSON parse failure (unclosed brackets, garbage payloads) under the `ERROR=invalid pieces.json: <reason>` stderr prefix + exit 1 contract; (b) valid JSON whose top-level value is not an array (object root with ≥2 keys, string root, etc.) under the same `ERROR=invalid pieces.json:` prefix; (c) per-entry `depends_on` containing a non-integer number (e.g. `[1.5]`) under the `ERROR=pieces.json entry <i> has out-of-range depends_on values:` prefix + exit 1 — pinned by case 7, closes #647; (d) per-entry `title` containing an embedded LF under the `ERROR=pieces.json entry <i> title contains embedded newline` prefix + exit 1 — pinned by case 8, closes #648; and (e) `--tmpdir` directory exists but is not writable, under the `ERROR=tmpdir not writable: <path>` prefix + exit 1 — pinned by the `assert_unwritable_tmpdir` case, closes #687. Without the case-(d) guard, a multi-line `title` would later be emitted via `printf 'PIECE_<i>_TITLE=%s\n' "$title"` and split one logical KV into multiple physical stdout lines, silently breaking the one-KV-per-line grammar that `umbrella` SKILL.md Step 3B.1 parses. Without the case-(e) guard, an unwritable `--tmpdir` would surface as a raw bash "Permission denied" on the first redirect under `$TMPDIR` (`2>"$JQ_PARSE_ERR"` or `: > "$OUT"`) — same bug class as #645 on `render-umbrella-body.sh`. The remaining per-entry shape validation (`title` / `body` non-empty) is covered by integration via `SKILL.md` end-to-end runs.
