...
6. **Anti-halt continuation reminder** (orchestrators only — closes #177): if this skill runs additional steps after a child Skill call returns, include the canonical top-of-file banner and per-call-site micro-reminders from \`${PLUGIN_TOKEN}/skills/shared/subskill-invocation.md\` section Anti-halt continuation reminder. Pure delegators (\`/im\`, \`/imaq\`, \`/alias\`, \`/create-skill\`) are exempt. The banner and micro-reminder substrings are asserted by \`${PLUGIN_TOKEN}/scripts/test-anti-halt-banners.sh\` (wired into \`make lint\` via the \`test-anti-halt\` target).
