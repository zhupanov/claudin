...
argument-hint: "[--plugin] [--multi-step] [--merge] [--no-slack] <skill-name> <description>  (--merge is a backward-compat no-op; /im auto-merges)"
...
Print: `**Create-skill /<NAME> (<plugin-dev|consumer>, <minimal|multi-step>) — delegating to /im --quick --auto [--no-slack]**` (omit `--no-slack` when `NO_SLACK=false`). `/im` auto-merges; `--merge` on `/create-skill` is a backward-compat no-op and is not forwarded.

Invoke the Skill tool:
- Try skill: `"im"` first (bare name). If no skill matches, try skill: `"larch:im"` (fully-qualified plugin name).
- args: `"--quick --auto [--no-slack] <feature-description>"` — include `--no-slack` only if `NO_SLACK=true`. `--merge` is NOT forwarded (`/im` prepends it itself); the `MERGE` parse value is ignored at delegation time.
