...
| `$LOOP_TMPDIR/iter-N-out.txt` | Per-iteration `claude -p /fix-issue` stdout in **stream-json (NDJSON) format**, raw and unredacted, exactly one file per iteration. One JSON object per line covering `system` init, `assistant` turns (with `.message.content[*].text` carrying the breadcrumb text verbatim — `&` and em-dash are NOT escaped), `tool_use` events, and the final `result`. Termination-detection grep reads this file (not driver stdout) with `grep -aF` for the literal sentinel substring; the literal still matches because Anthropic's stream-json encoder preserves the breadcrumb bytes verbatim within JSON string fields. | Wiped by `cleanup-tmpdir.sh` when `LOOP_PRESERVE_TMPDIR=false` (clean success and `--max-iterations` cap-hit). Retained when `LOOP_PRESERVE_TMPDIR=true` (the four documented abnormal-exit paths). On retained paths, the cleanup warning names the artifact glob patterns. |
