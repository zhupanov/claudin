...
- **Per-iteration child stdout/stderr**: each iteration's `claude -p /fix-issue` runs with `--output-format stream-json --verbose`, so `$LOOP_TMPDIR/iter-N-out.txt` is a newline-delimited stream of JSON objects (one per `system` init / `assistant` turn / `tool_use` event / `result`). The Step 0 success breadcrumb appears verbatim somewhere in the NDJSON sidecar — typically inside an `assistant`-typed JSON line's text content — and the driver's `grep -aF` for the literal sentinel substring matches against any occurrence in the file because Anthropic's stream-json encoder preserves `&`, the em-dash, and other ASCII / UTF-8 content verbatim within JSON string fields. The driver does NOT parse the JSON structure. Raw stderr (including any `claude-iter-N: TIMED OUT after Xs` watcher diagnostic) lands in `$LOOP_TMPDIR/iter-N-out.txt.stderr`. Both files are retained **only** when `LOOP_PRESERVE_TMPDIR=true`, which the driver sets on every documented abnormal-exit path: claude subprocess error, Step 0 error, Step 0 lock failure, and sentinel mismatch. On clean success and on `--max-iterations` cap-hit (also a clean exit), `LOOP_TMPDIR` is wiped and the sidecars go with it. On retained paths, the cleanup warning line names the artifact glob patterns explicitly so they can be located without consulting these docs. **Privacy note**: NDJSON sidecars include `tool_use` event payloads and the full per-turn assistant text, which is materially more verbose than the prior default-mode "final assistant message only" capture; treat retained sidecars like other raw debug logs (see `SECURITY.md` `## /loop-fix-issue subprocess invocation`).
