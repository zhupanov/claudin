...
argument-hint: "[--slack] [--issue <N>] <skill-name>"
...
- `--slack`: When present before `<skill-name>`, forwarded to the `/larch:im` invocation inside the iteration so the PR posts to Slack. Default: absent — `/larch:im` does not post to Slack regardless of Slack env-var presence.
