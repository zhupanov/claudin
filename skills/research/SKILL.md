---
name: research
description: "Use for read-only research. --scale=standard|deep → 3/5 lanes; --quick → K=3 Claude lanes vote-merged; --plan/--interactive add planner pre-pass; --adjudicate, --keep-sidecar, --token-budget supported."
argument-hint: "[--debug] [--plan] [--interactive] [--scale=standard|deep] [--quick] [--adjudicate] [--keep-sidecar[=PATH]] [--token-budget=N] <research question or topic>"
...
Flags are independent — the presence of one flag must not influence the default value of any other flag. `--debug`, `--interactive`, `--scale`, `--quick`, `--adjudicate`, `--keep-sidecar`, and `--token-budget` are independent and may appear in any order at the start of `$ARGUMENTS`. (The --plan flag is coupled — its bullet documents a --scale cross-effect resolution rule, see "Planner pre-pass — scale interaction" below.) The single mutual-exclusion gate is --quick + explicit --scale=standard|deep (see --quick below); the gate is enforced after parsing completes, not as an inter-flag default override. --interactive requires --plan (usage error if --interactive is set without --plan).
...
- `--interactive` (boolean): Set a mental flag `RESEARCH_PLAN_INTERACTIVE=true`. Requires `--plan`; pauses after the planner pre-pass (Step 1.1) so the operator can review the 2–4 proposed subquestions before fan-out. Operator types Enter (proceed), `edit` (revise via `$EDITOR` or stdin fallback), or `abort` (exit cleanly). Hard-fails before any planner work when stdin is not a TTY. Deep mode confirms only the subquestion list — the per-lane subq×angle pairing stays mechanical (Step 1.2 unchanged). Default: `RESEARCH_PLAN_INTERACTIVE=false` (byte-equivalent to pre-#522 behavior). See "Planner pre-pass — scale interaction" below for the resolution rule (interaction with `--plan` and `--quick` — `--quick` disables `--plan` so `--interactive` is also a no-op under `--quick`).
- `--scale=standard|deep` (value): Set a mental flag `RESEARCH_SCALE` to the explicitly-provided value AND set a second mental flag `SCALE_EXPLICIT=true` (used by the mutual-exclusion gate below). Default: `RESEARCH_SCALE=standard` AND `SCALE_EXPLICIT=false` (no explicit form). Selects the lane shape (3+3 / 5+5) for the research and validation phases — see "Scale matrix" below. Reject malformed forms with explicit error and abort: `--scale=quick` (legacy value, removed in favor of the orthogonal `--quick` boolean) → print `**⚠ /research: --scale=quick has been replaced by --quick (boolean). Use --quick for K=3 vote-merged Claude lanes. Aborting.**` and exit; `--scale=foo` (unknown value) → print `**⚠ /research: --scale must be one of standard|deep (got: foo). Aborting.**` and exit; `--scale` without `=value` → print `**⚠ /research: --scale requires a value (standard|deep). Aborting.**` and exit; `--scale=` (empty value) → same error as missing value.
- `--quick` (boolean): Set a mental flag `RESEARCH_QUICK=true`. Default: `RESEARCH_QUICK=false`. When `RESEARCH_QUICK=true`, Step 1 runs `RESEARCH_QUICK_K=3` (hardcoded) homogeneous Claude inline lanes via the Agent tool (`subagent_type=general-purpose`) carrying identical `RESEARCH_PROMPT_BASELINE` plus a structured-output rubric instructing each lane to emit a `<lane_claims>` block with 3-7 atomic claims (claim text, evidence file:line citations, confidence high|medium|low, unknowns), per-lane outputs go to `$RESEARCH_TMPDIR/quick-lane-{1,2,3}-output.txt`, and Step 1.5 vote-merges via reproducible Jaccard-based clustering (≥0.6 Jaccard over normalized claim keys) — see `${CLAUDE_PLUGIN_ROOT}/skills/research/references/research-phase.md` Step 1.3/1.4/1.5 Quick subsections. Step 2 (validation panel) is skipped — same skip-gate semantics as the prior `--scale=quick` path, just re-keyed to `RESEARCH_QUICK=true`. Three-tier partial-failure handling: 3/3 lanes succeed → full vote-merge; 2/3 → degraded vote-merge with explicit "1 lane failed (reason)" disclaimer prepended to synthesis; <2 (0/3 or 1/3) → fail-closed (abort run with explicit error). An internal `quick-ballot.md` artifact with parsed per-lane claims and cluster mapping is written to `$RESEARCH_TMPDIR`; preserved alongside the findings sidecar at Step 4 when `KEEP_SIDECAR=true` (using the same destination validation guardrails). **Mutual-exclusion gate** (enforced AFTER all flags are parsed, before Step 0 setup): if `RESEARCH_QUICK=true` AND `SCALE_EXPLICIT=true`, print `**⚠ /research: --quick is mutually exclusive with --scale=standard|deep (those modes already provide cross-tool heterogeneity = implicit voting). Aborting.**` and exit. The implicit default (`RESEARCH_SCALE=standard` with `SCALE_EXPLICIT=false`) does NOT conflict with `--quick`. **`--plan + --quick` interaction**: see "Planner pre-pass — scale interaction" below — `--plan` is downgraded to `false` with a warning citing the homogeneous-lane requirement.
- `--adjudicate` (boolean): Set a mental flag `RESEARCH_ADJUDICATE=true`. When set, runs a 3-judge dialectic adjudication after Step 2's Finalize Validation over every reviewer finding the orchestrator rejected during validation merge/dedup — see Step 2.5 below. THESIS = "rejection stands"; ANTI_THESIS = "reinstate the reviewer's finding"; majority binds. Default: `RESEARCH_ADJUDICATE=false` (Step 2.5 short-circuits with `⏩` and behavior is unchanged from prior versions). The `(finding, rejection_rationale)` capture in Step 2 runs unconditionally (regardless of this flag), but writes only to tmpdir scratch — when the flag is off, no extra LLM work, no external-tool launches, and no additional user-visible output is produced. Composes cleanly with `--quick` (which skips Step 2 entirely): when both are set, Step 2.5 short-circuits with `⏩ no rejections to adjudicate (--quick skipped Step 2)` since `rejected-findings.md` is never written.
...
`--quick` and `--scale` are orthogonal mode selectors. The runtime mode is `--quick` when `RESEARCH_QUICK=true` (overriding the implicit `RESEARCH_SCALE=standard` default — `--quick` and `--scale=standard|deep` cannot both be explicitly set, per the mutual-exclusion gate). When `RESEARCH_QUICK=false`, the active mode is `RESEARCH_SCALE` (default `standard`).

| Mode | Step 1 (research) lanes | Step 2 (validation) lanes |
|---|---|---|
| `--quick` (`RESEARCH_QUICK=true`) | 3 (homogeneous Claude inline lanes — `RESEARCH_QUICK_K=3` hardcoded; identical `RESEARCH_PROMPT_BASELINE`; vote-merged via Jaccard clustering — K-lane vote-merge confidence) | 0 (Step 2 skipped at SKILL.md gate) |
| `RESEARCH_SCALE=standard` (default) | 3 (Cursor → ARCH + Codex → EDGE/EXT + Claude inline → SEC, angle-differentiated) | 3 (Code + Cursor + Codex) |
| `RESEARCH_SCALE=deep` | 5 (Claude inline + Cursor-Arch + Cursor-Edge + Codex-Ext + Codex-Sec, with named angle prompts) | 5 (Code + Code-Sec + Code-Arch + Cursor + Codex) |
...
`--plan` is supported with `--scale=standard` (the default 3-lane shape) and `--scale=deep` (the 5-lane shape with named angle prompts). With `--quick` (K=3 homogeneous Claude lanes), downgrade `--plan` to `false` with a visible warning at the start of Step 1 — do NOT silently ignore the flag, and do NOT reject the run. Resolution rule (applied before Step 1 begins):

- `RESEARCH_PLAN=true` AND `RESEARCH_QUICK=true`: print `**⚠ /research: --plan is not applicable to --quick (K=3 homogeneous Claude lanes require identical RESEARCH_PROMPT_BASELINE across all lanes for vote-merge — subquestion decomposition would split lanes across different questions, defeating the consensus comparison). Disabling --plan for this run.**`, set `RESEARCH_PLAN=false`, continue.
...
3. Apply the `--interactive` resolution against the post-step-2 `RESEARCH_PLAN` value:
   - `RESEARCH_PLAN_INTERACTIVE=true` AND `--plan` was set originally AND `RESEARCH_PLAN=false` after step 2 (i.e., disabled by `--quick` per the homogeneous-lane requirement): print `**⚠ /research: --interactive ignored (--plan was disabled by --quick).**`, set `RESEARCH_PLAN_INTERACTIVE=false`, continue.
...
(Step 1.1 and 1.2 are sub-steps of Step 1 that execute only when `RESEARCH_QUICK=false` AND `RESEARCH_PLAN=true`. Step 1.1.c additionally requires `RESEARCH_PLAN_INTERACTIVE=true`. They are skipped on every other path — `--quick` mode requires homogeneous lanes (identical `RESEARCH_PROMPT_BASELINE` across K=3) for vote-merge, so per-lane subquestion suffixes would defeat the consensus comparison.)
...
**Skip this entire sub-step when `RESEARCH_QUICK=true`.** Quick-mode (K=3 homogeneous Claude lanes) has no external lanes to attribute; Step 3 emits a literal "3 agents (homogeneous Claude inline — K-lane vote-merge confidence)" research-phase header and the literal `0 reviewers (validation phase skipped — see synthesis disclaimer)` validation-phase header, without consulting `lane-status.txt` (Step 3 Quick branch sets these literals directly — see the `### Quick (RESEARCH_QUICK=true)` subsection below).

For `RESEARCH_QUICK=false` AND (`RESEARCH_SCALE=standard` OR `RESEARCH_SCALE=deep`), write `$RESEARCH_TMPDIR/lane-status.txt` with the per-tool aggregate pre-launch attribution. The same 8-key schema below covers both standard (1 Cursor + 1 Codex per phase) and deep (per-tool aggregate across 2 Cursor + 2 Codex in research, 1 Cursor + 1 Codex in validation): `RESEARCH_CURSOR_*` reflects the per-tool aggregate over both Cursor research slots in deep mode; same for `RESEARCH_CODEX_*`. Step 1.4 (research-phase) and Step 2 entry / validation-phase render-failure handlers (Cursor / Codex `On non-zero exit` paths) / Step 2.4 (validation-phase) update this file later via surgical phase-local rewrites; Step 3 reads it via `${CLAUDE_PLUGIN_ROOT}/scripts/render-lane-status.sh` for standard mode and `${CLAUDE_PLUGIN_ROOT}/scripts/render-deep-lane-status.sh` for deep mode to render the final-report header (both share the rendering library `render-lane-status-lib.sh`). Quick mode does NOT consult this file — see Step 3 ### Quick.
...
When `RESEARCH_TOKEN_BUDGET` is non-empty AND `RESEARCH_QUICK=false`, print: `**ℹ --token-budget=$RESEARCH_TOKEN_BUDGET governs measured Claude subagent tokens only; Claude inline + external lanes (Cursor/Codex) are excluded from the cap. Unmeasurable lanes are reported separately.**`. When `RESEARCH_QUICK=true`, the K=3 Claude subagents ARE measurable (each Agent return carries a `<usage>` block), so emit a quick-specific notice: `**ℹ --token-budget=$RESEARCH_TOKEN_BUDGET governs the K=3 Claude lane subagents (each emits a usage block); the orchestrator's vote-merge clustering is unmeasurable Claude inline.**`.
...
**Quick-mode skip gate (emitted FIRST, before any reference load — Check 3 of `scripts/test-research-structure.sh` requires the MANDATORY directive line below to remain on a single line carrying both the directive and the reciprocal `Do NOT load` guards, so the skip gate is structured to short-circuit BEFORE that line)**: if `RESEARCH_QUICK=true`, print `⏩ 2: validation — skipped (--quick) (<elapsed>)` and proceed directly to Step 3 without loading `validation-phase.md`. The K-lane vote-merged research-report.txt produced at Step 1.5 is the canonical input to Step 3.
...
Skip `render-lane-status.sh` (Step 0b did not write `lane-status.txt` for quick mode). Emit literal headers depending on the Step 1 partial-failure tier (`QUICK_LANES_OK` ∈ {3, 2}; the <2 fail-closed tier aborts the run before Step 3):

- 3/3 lanes succeeded: `RESEARCH_HEADER="3 agents (homogeneous Claude inline — K-lane vote-merge confidence)"`.
- 2/3 lanes succeeded (degraded): `RESEARCH_HEADER="2 agents (homogeneous Claude inline — K-lane vote-merge confidence; 1 lane failed: $QUICK_LANE_FAIL_REASON)"`.
- `VALIDATION_HEADER="0 reviewers (validation phase skipped — see synthesis disclaimer)"`.

The validation-phase line is still rendered (with the 0-reviewers literal) so the report template's structure is preserved. The synthesis itself must already carry the explicit "K-lane vote-merge confidence" disclaimer per `research-phase.md` Step 1.5 Quick branch (with the degraded-mode "1 lane failed" prefix prepended when `QUICK_LANES_OK=2`).
...
Quick mode (`RESEARCH_QUICK=true`) does NOT carry this banner — it has its own `**K-lane vote-merge confidence — no validation pass.**` disclaimer (sourced from `${CLAUDE_PLUGIN_ROOT}/skills/research/data/quick-disclaimer.txt`) that already signals the lower-assurance outcome.
...
After the write succeeds, invoke the helper to generate the sidecar. Source the Quick-mode disclaimer from the canonical file `${CLAUDE_PLUGIN_ROOT}/skills/research/data/quick-disclaimer.txt` only when `RESEARCH_QUICK=true` (#510 design FINDING_4 — re-keyed from `RESEARCH_SCALE=quick` to `RESEARCH_QUICK=true` per the `--quick` flag migration). The empty-array safe expansion `${ARR[@]+"${ARR[@]}"}` (quotes inside the `+` alternative — #510 review FINDING_1) is REQUIRED so a multi-element array does not collapse into one fused positional argument when the disclaimer text contains spaces:
...
### Preserve quick-mode ballot (when `KEEP_SIDECAR=true` AND `RESEARCH_QUICK=true`)

When both `KEEP_SIDECAR=true` and `RESEARCH_QUICK=true`, also copy the quick-mode ballot artifact alongside the findings sidecar. The ballot is `$RESEARCH_TMPDIR/quick-ballot.md` (written by Step 1.5 Quick — see `${CLAUDE_PLUGIN_ROOT}/skills/research/references/research-phase.md`) and contains each lane's parsed `<lane_claims>` plus the cluster mapping the orchestrator used for vote-merge synthesis.

Destination derivation: take the resolved findings-sidecar `DEST` from the block above and append `-ballot.md` to its base (replacing a final `.md` suffix when present). Examples: `./research-findings-batch.md` → `./research-findings-batch-ballot.md`; `--keep-sidecar=/foo/bar.md` → `/foo/bar-ballot.md`; `--keep-sidecar=/foo/bar` (no `.md`) → `/foo/bar-ballot.md`.

**Path validation reuses the same destination guardrails as the findings sidecar `cp`** (FINDING_7) — must NOT resolve under `$RESEARCH_TMPDIR`, parent directory must exist and be writable, `realpath` is the security-stronger tier when available, fall back to a string-prefix check on `cd "$(dirname …)" && pwd` resolution. If validation fails, print `**⚠ 4: cleanup — quick-ballot destination rejected: <reason>. Ballot will be cleaned up with the tmpdir.**` and SKIP the `cp`; do NOT abort.

```bash
if [[ "$KEEP_SIDECAR" == "true" ]] && [[ "$RESEARCH_QUICK" == "true" ]]; then
  BALLOT="$RESEARCH_TMPDIR/quick-ballot.md"
  if [[ -f "$BALLOT" ]]; then
    # Derive BALLOT_DEST from DEST (strip trailing .md if present, append -ballot.md).
    case "$DEST" in
      *.md) BALLOT_DEST="${DEST%.md}-ballot.md" ;;
      *) BALLOT_DEST="${DEST}-ballot.md" ;;
    esac
    # Validate BALLOT_DEST per the same rules as DEST; assign BALLOT_DEST_OK=true/false.
    if [[ "$BALLOT_DEST_OK" == "true" ]]; then
      if cp "$BALLOT" "$BALLOT_DEST"; then
        echo "**📋 Quick-mode ballot preserved at $BALLOT_DEST. Inspect to audit which lane claims clustered into Agreements vs Divergences.**"
      else
        echo "**⚠ 4: cleanup — cp to $BALLOT_DEST failed; ballot will be cleaned up with the tmpdir.**"
      fi
    fi
  fi
fi
...
The ballot file is operator-only diagnostic content (per-lane claims + cluster mapping) — it is NOT a `/issue` batch input and the `--dry-run` advertisement does not apply.
