...
**Contract**: dialectic adjudication of orchestrator-rejected reviewer findings. Owns the conditional skip-path on empty `rejected-findings.md`, the pre-launch coordinator invocation (`scripts/run-research-adjudication.sh`), the 3-judge panel launch and collection (replacement-first when externals are unhealthy at the coordinator's fresh probe), the dialectic-protocol.md parser-tolerance + threshold-rule reuse, the `adjudication-resolutions.md` schema (pinned to dialectic-protocol.md's Consumer Contract field names), and the reinstatement-into-validated-synthesis sub-step. Consumes `$RESEARCH_TMPDIR/rejected-findings.md` (written unconditionally at validation-phase.md Sites A and B) and `$RESEARCH_TMPDIR/research-report.txt` (the validated synthesis from Step 2's Finalize Validation, body under the `## Revised Research Findings` header — falling back to Step 1.4's `## Research Synthesis` header when no findings were accepted at validation). Produces `$RESEARCH_TMPDIR/research-adjudication-ballot.txt` (the dialectic ballot) and `$RESEARCH_TMPDIR/adjudication-resolutions.md` (the audit trail).

**When to load**: once Step 2.5 is about to execute AND `RESEARCH_ADJUDICATE=true`. Do NOT load during Step 0, Step 1, Step 2, Step 2.7, Step 3, or Step 4. SKILL.md emits the Step 2.5 entry breadcrumb and the Step 2.5 completion print; this file does NOT emit those — it owns body content only.
...
Implementer checklist (post-edit verification — each item describes a failure mode the implementer rules out by scanning **executable bash code-fenced blocks** in this file; prose cross-references are deliberately exempt because they may legitimately mention the design-context names in explanatory context):

1. **Stray design-context tmpdir variable**: scan executable bash blocks for the design-session tmpdir variable (the one removed from the shared protocol by issue #440). The expected result is zero matches. A hit indicates wrong file or an incomplete copy from the design context. (This checklist line deliberately does not spell the literal token so the grep stays mechanically reliable; the prose mention here is explanatory only.)
2. **Wrong ballot filename**: scan executable bash blocks for the design-context ballot filename (the one written by `/design`'s dialectic execution — see `${CLAUDE_PLUGIN_ROOT}/skills/design/references/dialectic-execution.md`). The expected result is zero matches. A hit indicates the ballot path is pointing at the design-context ballot rather than the research-specific `research-adjudication-ballot.txt` — judges would read a non-existent path. (As with item 1, the design-context filename is referenced indirectly here so the grep stays mechanical against bash blocks. Documentation cross-references in prose elsewhere in this file — e.g., the substitution note above and the field-rules section below — are expected and not failure cases.)
3. **Wrong output filename**: scan executable bash blocks for the design-context resolutions filename — a *different* design-context token from item 2's ballot filename (this is the output audit-trail artifact, not the input ballot; see `${CLAUDE_PLUGIN_ROOT}/skills/design/references/dialectic-execution.md` for the design-context resolutions filename). The expected result is zero matches. A hit indicates the output path is pointing at the design artifact rather than the research-specific `adjudication-resolutions.md` — this is a wrong-output-path failure (the entire artifact would be misnamed), distinct from a basename-substitution slip. (Same prose-mention rule as item 2.)
...
**`RAN=false` branch**: the coordinator detected an empty/absent `rejected-findings.md`. Proceed to SKILL.md's `⏩ 2.5: adjudication — no rejections to adjudicate` print and return to Step 2.7 without launching judges. Do NOT write `adjudication-resolutions.md`.
...
1. Locate the original reviewer finding in `$RESEARCH_TMPDIR/rejected-findings.md`. **`DECISION_N` is NOT the same as the literal `<N>` suffix from `### REJECTED_FINDING_<N>`** — the former is the post-sort index produced by the ballot builder; the latter is the append-order session counter from validation-phase.md Sites A and B. They only coincide when the rejection capture order happens to match `(reviewer_attribution, sha256(finding_text))` lex order. To find the correct block, apply the algorithm:
   ...
   By the time this step runs, the ballot builder guarantees that **every parsed block is complete** (it fails closed on any block missing one of `Reviewer`, `Finding`, or `Rejection rationale` — see `scripts/build-research-adjudication-ballot.md` "Incomplete-record handling"). The algorithm above therefore does not need a separate completeness filter; any path that reaches this step has a one-to-one correspondence between sorted blocks and `DECISION_k` slots on the ballot. Issue #462 documents the prior bug class (silent soft-drop in the builder created a `DECISION_k → REJECTED_FINDING_<N>` mismatch) and the fail-closed fix that eliminates it.
...
- **Pre-launch coordinator fails** (`run-research-adjudication.sh` exits non-zero): print `**⚠ 2.5: adjudication — coordinator failed: <ERROR>. Skipping adjudication.**` and proceed to Step 2.7 without further action. The orchestrator's pre-adjudication validation outcome stands; rejected-findings.md is preserved in tmpdir until Step 4 cleanup.
- **Ballot builder fails inside the coordinator** (script exits non-zero with `FAILED=true`): same recovery — coordinator surfaces the error in stdout; Step 2.5 skips with a warning and Step 2.7 proceeds.
- **Incomplete `### REJECTED_FINDING_<N>` block in `rejected-findings.md`**: the ballot builder fails closed (exit 2) on any block missing `Reviewer`, `Finding`, or `Rejection rationale` (whitespace-only field bodies are treated as missing). The coordinator pattern-matches the builder's `ERROR=` line and prepends an `incomplete-input:` tag, surfacing `RAN=false / FAILED=true / ERROR=incomplete-input: REJECTED_FINDING_<N> is incomplete...`. Same recovery as the generic coordinator-failure case above — print the warning and skip adjudication. The orchestrator's pre-adjudication validation outcome stands; the malformed `rejected-findings.md` is preserved in tmpdir for diagnostic inspection.
