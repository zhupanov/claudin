...
3. **Trivial files** — If the file is `version.go`, `go.sum`, `.claude-plugin/plugin.json`, any file listed in `LARCH_BUMP_FILES` (when set), or auto-generated, classify as **trivial** and auto-resolve immediately. Stage with `${CLAUDE_PLUGIN_ROOT}/scripts/git-stage.sh <file>`. For `.claude-plugin/plugin.json` and any file in `LARCH_BUMP_FILES`, resolve to the **upstream (main) version** via `${CLAUDE_PLUGIN_ROOT}/scripts/git-checkout-ours.sh <file>` — during rebase, `--ours` refers to the base being rebased onto, i.e., upstream main, because the Rebase + Re-bump Sub-procedure will overwrite bump files with a fresh bump in its step 4 after the rebase completes. See the note below.
...
5. **Classify confidence**:
   - **Trivial**: `version.go`, `go.sum`, `.claude-plugin/plugin.json`, files listed in `LARCH_BUMP_FILES` (when set), auto-generated files.
