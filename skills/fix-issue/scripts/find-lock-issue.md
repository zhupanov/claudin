...
1. **Find** — eligibility scan (auto-pick mode) or explicit-issue verification. Existing logic preserved for the non-umbrella path: open state, GO sentinel as last comment, no `IN PROGRESS` lock present, no managed lifecycle title prefix (`[IN PROGRESS]` / `[DONE]` / `[STALLED]`), and no open blocking dependencies (native + prose).

   **Auto-pick selection order**: the open-issues list is sorted by a two-key composite (jq `sort_by([(.title // "" | ascii_downcase | contains("urgent") | not), .number])`) so that issues with `Urgent` anywhere in the title (case-insensitive substring) are evaluated FIRST, with oldest-first by issue number as the within-tier tiebreaker. The preference is a soft signal — it only re-orders evaluation, not eligibility. Explicit-target mode (positional `<number-or-url>` provided) is unaffected; it always targets exactly the issue named.
