...
Every fixture asserts the script's exit code AND its stderr behavior in addition to the stdout key/value substring matches — stdout, stderr, and `$?` are captured separately by the `run_script_capture` helper, and the per-invocation `|| EC=$?` pattern preserves the exit code under `set -e` rather than masking it with `|| true`.

1. **finalize-success** — open umbrella, no existing marker, rename + close succeed → exit 0; stdout `FINALIZED=true RENAMED=true CLOSED=true`; stderr empty.
2. **marker-present-OPEN-close-only-retry** (FINDING_3) — comment stream contains the literal marker `<!-- larch:fix-issue:umbrella-finalized -->` while `state=OPEN` and the title carries no `[DONE]` prefix (prior comment-post succeeded but close did not) → rename runs as usual, ONLY the comment-post is skipped (avoid double-comment under concurrency), then close → exit 0; stdout `FINALIZED=true RENAMED=true CLOSED=true` and explicitly does NOT contain `ALREADY_FINALIZED=true`; stderr empty. Without the marker probe driving this close-only retry, the umbrella would stay stuck OPEN with every retry no-op.
3. **DONE-title-OPEN-close-only-retry** (FINDING_3) — title starts with the managed prefix `[DONE]` (followed by a space) while `state=OPEN` (prior rename succeeded but close did not) → close-only retry, skip the rename API call (do not re-rename) → exit 0; stdout `FINALIZED=true RENAMED=false CLOSED=true` and explicitly does NOT contain `ALREADY_FINALIZED=true`; stderr empty.
4. **idempotent-when-already-CLOSED** — state is `CLOSED` (any title, any comments) → strict short-circuit, the only one of the three guard signals that does → exit 0; stdout `FINALIZED=false ALREADY_FINALIZED=true REASON=already CLOSED`; stderr empty.
5. **rename-failed-but-close-success** — best-effort rename invariant: rename delegate fails (stubbed `gh issue edit` returns exit 1), but close succeeds → exit 0; stdout `FINALIZED=true RENAMED=false CLOSED=true`; stderr contains the literal `WARNING: title rename to [DONE] failed`. The lock is the correctness boundary; the title prefix is a visual lifecycle marker.
...
- **Exit-code contract** — every fixture currently expects exit 0 (the script returns 0 on both the executed-success path and the idempotent short-circuit). New non-idempotent failure fixtures (e.g., `CLOSE_FAIL=true`) MUST assert their expected exit code (1 for failures, 2 for usage errors) via `assert_eq_exit`.
- **Stderr contract** — fixtures 1-4 assert stderr is empty (the script suppresses underlying `gh` stderr via `2>/dev/null` on read paths and captures inner-script stderr into stdout-bound capture variables). Fixture 5 asserts stderr contains the `WARNING: title rename to [DONE] failed` literal emitted by `cmd_finalize` when the rename delegate fails. Changing where the script writes diagnostic messages requires updating these assertions.
...
- **Helper set** — `assert_contains` / `assert_not_contains` / `assert_eq_exit` / `assert_stderr_contains` / `assert_stderr_empty` plus the `run_script_capture` runner. `run_script_capture` is the single canonical entry point for invoking the script under test; new fixtures must use it (rather than ad-hoc `2>&1` merging) so the per-channel + exit-code contract stays uniform.
