...
7. **NEVER auto-pick umbrellas in the no-arg find-lock-issue scan.** **Why**: the umbrella-PR design dialectic (DECISION_1, voted 2-1 ANTI_THESIS) chose explicit-target-only umbrella handling. Auto-pick must keep its GO-tail invariant unchanged — folding umbrella handling into the bulk sweep multiplies decision-surface complexity (umbrella resolution is a distinct state machine with non-GO locking, child-pick semantics, and finalization paths) and increases operator surprise (umbrellas can be passive long-lived planning trackers). **How to apply**: `umbrella-handler.sh` is invoked from two distinct sites with different policies. The **explicit-issue** path uses it as a hard validation gate (fail-closed: missing helper, `detect` non-zero, or `detect` exit-0 with no recognized `IS_UMBRELLA=` value all abort with exit 2 — closes #765). The **auto-pick** path uses it ONLY to skip umbrella candidates fail-open (a broken helper does not stall the queue). Auto-pick never DISPATCHES umbrellas — that path remains explicit-target-only per DECISION_1. The auto-pick scan loop continues to require `GO` as the last comment for ANY candidate. Operators who want umbrella-tracked work to drain in `/loop-fix-issue` must explicitly pass the umbrella number (e.g., `/fix-issue <umbrella#>` once per dispatch cycle). **CI-backed**: no — the auto-pick-skips-umbrella regression fixture has not yet been added to `test-find-lock-issue.sh`; the explicit-issue fail-closed gate IS CI-backed via Fixtures 10-12 there. (The original "yes" claim referred to a fixture that did not exist; corrected per #765 plan-review FINDING_4.)
...
**Explicit-mode umbrella-validation gate** (closes #765): in explicit-issue mode, umbrella detection is a hard validation gate — `find-lock-issue.sh` aborts with exit 2 (`ELIGIBLE=false` + `ERROR=...` on stdout, human-readable diagnostic on stderr) when `umbrella-handler.sh` is missing/non-executable, when `umbrella-handler.sh detect` exits non-zero, OR when `detect` exits 0 but emits no recognized `IS_UMBRELLA=` value. This closes a fail-open hole where an unverifiable umbrella-detect would silently fall through to the GO-tail check and (if the umbrella's tail comment was `GO`) lock the umbrella issue itself or pass its body downstream as a feature spec. Auto-pick mode keeps a separate fail-OPEN umbrella-skip block to preserve queue progress when the helper is broken; the two policies are deliberately different. See `find-lock-issue.md` "Explicit-mode fail-closed semantics" for the full contract.
