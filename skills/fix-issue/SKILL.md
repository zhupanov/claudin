...
**Where does a crash leave the issue?** `IN PROGRESS` is a lock, not a status. A Step 1+ crash keeps the issue locked until a human clears the comment. Consult Known Limitations for each recovery path before deviating from the step sequence.
