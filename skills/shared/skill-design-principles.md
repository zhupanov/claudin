...
- **A — Express content and logic as bash scripts.** Any non-trivial step belongs in a `.sh` file: shared at `${CLAUDE_PLUGIN_ROOT}/scripts/` when two or more skills can reuse it, or private at `${CLAUDE_PLUGIN_ROOT}/skills/<NAME>/scripts/` when it is skill-specific. Prefer reuse — grep existing `scripts/` before creating a new one. See the owning script's sibling `.md` (co-located contract; convention described in `AGENTS.md § Editing rules`). For shared helpers without their own `.md` (e.g., sourced-only libraries), the contract lives under the primary script's sibling `.md`.
