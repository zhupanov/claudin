...
**Scope**: this rule applies to **orchestrators that continue execution based on a child skill's side effects** — e.g., a parent that reads the child's output to decide the next step. Pure forwarders (`/im`, `/imaq`, `/create-skill`, `/loop-improve-skill`, `/improve-skill`, `/simplify-skill`, `/compress-skill`) are exempt — once they delegate, they do nothing further, so there is nothing to verify.
...
**Scope**: this rule applies to the same orchestrator set as `## Post-invocation verification` above — stateful orchestrators (`/fix-issue`, `/implement`, `/review`, `/alias`, `/research`) that run additional steps after a child `Skill` tool call returns. Pure forwarders (`/im`, `/imaq`, `/create-skill`, `/loop-improve-skill`, `/loop-review`, `/improve-skill`, `/simplify-skill`, `/compress-skill`) are exempt — once they delegate, they do nothing further. (`/loop-improve-skill`, `/loop-review`, and `/improve-skill` are classified as pure delegators because their runtime drivers live under `skills/loop-improve-skill/scripts/driver.sh`, `skills/loop-review/scripts/driver.sh`, and `skills/improve-skill/scripts/iteration.sh` respectively — all are subprocess-driven bash scripts that do not use the `Skill` tool to chain children.) The two sections are complementary: `## Post-invocation verification` asks **"did the child run?"**; this section asks **"did the parent continue?"** Both failure modes are distinct and real (see GitHub issue #177 for the originating report).
...
**Loop-internal carve-out**: when an orchestrator's step explicitly loops (a hypothetical Skill-tool call inside a loop body), the "next step" of the parent IS the loop-continuation directive, not the first textually-following section header. Use the loop-aware micro-reminder variant at loop-internal child-Skill call sites. (Note: the historical `/loop-review` example moved to bash-driver inversion-of-control post-overhaul; loop control now lives in `driver.sh`, not in the SKILL.md prompt — so this carve-out applies only to currently-orchestrator skills with future loop bodies.)
...
This file is the canonical source for sub-skill invocation conventions (Pattern A bulleted vs Pattern B inline, `allowed-tools` narrowing heuristic, post-invocation verification for orchestrators, anti-halt continuation reminder for orchestrators (closes #177), `session-env` handoff and safe-parse rule, anti-conditional-phrasing for Skill-tool calls, bare-name-then-fully-qualified fallback). Runtime surface (ships to consumers under `skills/`). No generated artifact — update directly. Update trigger: when a cited source-example skill (`/im`, `/imaq`, `/alias`, `/create-skill`, `/fix-issue`, `/implement`, `/review`, `/loop-review`) changes its invocation pattern or its anti-halt banner/micro-reminder, update the corresponding example in the guide in the same PR. `skills/create-skill/scripts/render-skill-md.sh` emits a `## Sub-skill Invocation` reminder block referencing this file into every scaffolded skill; `skills/create-skill/scripts/test-render-skill-md.sh` is the regression harness guarding that emission (wired into `make lint` via the `test-render-skill` target). `scripts/test-anti-halt-banners.sh` is the paired regression harness for the anti-halt banner and micro-reminder — it asserts banner presence in the five orchestrator SKILL.md files (`/fix-issue`, `/implement`, `/review`, `/alias`, `/research`; `/loop-review` was previously listed but reclassified as a delegator after the inversion-of-control overhaul which moved its loop into `skills/loop-review/scripts/driver.sh`, parallel to `/loop-improve-skill`'s post-#273 reclassification), absence in the eight pure-delegator SKILL.md files (`/im`, `/imaq`, `/create-skill`, `/loop-improve-skill`, `/loop-review`, `/improve-skill`, `/simplify-skill`, `/compress-skill`), and micro-reminder presence in each of the orchestrators. `/alias` is classified as an orchestrator because its Step 4 runs a sentinel-file verification after `/implement` returns. `/research` is classified as an orchestrator because it may invoke `/issue` via the Skill tool and continue to its report/cleanup steps. `/loop-improve-skill` is classified as a DELEGATOR post-#273 because it declares `Bash` (driver launch) and `Monitor` (passive observability of the driver log file — closes #291) in `allowed-tools` and forwards execution unconditionally to `${CLAUDE_PLUGIN_ROOT}/skills/loop-improve-skill/scripts/driver.sh`; loop control, subprocess invocation, grade parsing, audit-trail posting, and cleanup all live in the driver (not in the SKILL.md body or in any Skill-tool call chain). `Monitor` is a passive log-tailing transport (`tail -F` + `grep` over the driver's stdout/stderr log file at a `/tmp`-only path) that does NOT introduce any Skill-tool call chain and does NOT reclassify the skill as an orchestrator. Wired into `make lint` via the `test-anti-halt` target.
