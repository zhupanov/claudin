...
**Caller binding for shared dialectic protocol**: This file is the `/design` caller of `${CLAUDE_PLUGIN_ROOT}/skills/shared/dialectic-protocol.md`. The shared protocol writes its path placeholders in terms of `$DIALECTIC_TMPDIR`; before quoting any of the protocol's choreography (ballot path, judge prompt, judge launches, resolution path), `/design` binds **`DIALECTIC_TMPDIR=$DESIGN_TMPDIR`**. Existing `$DESIGN_TMPDIR` literals in this file resolve through that binding; the protocol's filename defaults (`dialectic-ballot.txt`, `dialectic-resolutions.md`, `cursor-judge-output.txt`, `codex-judge-output.txt`) are kept verbatim because `/design` is the protocol's design-context caller. External CLIs (Cursor/Codex) do not expand shell variables in prompt arguments, so any literal `$DIALECTIC_TMPDIR` token copied from the shared protocol is replaced at prompt-construction time with the bound `$DESIGN_TMPDIR` value (same convention as the orchestrator's pre-existing `$DESIGN_TMPDIR` substitution).
