---
name: alias
description: "Use when creating shortcut aliases (wrappers) for existing larch skills with preset flags. Generates a project-level skill in .claude/skills/ via /implement delegation, with optional --merge / --no-slack passthrough."
argument-hint: "[--merge] [--no-slack] <alias-name> <target-skill> [preset-flags...]"
...
5. **NEVER parse `--merge` or `--no-slack` tokens after the first positional argument as flags for `/alias`.** **Why:** both flags have a dual role (consumed by `/alias` when before the first positional; passed through to the alias's preset flags otherwise); conflating the two is a silent footgun.
...
**`--merge` / `--no-slack` dual-role reference**:

| Position | Meaning |
|----------|---------|
| Before first positional token | Consumed by /alias (`--merge` → `alias_merge=true`; `--no-slack` → `alias_no_slack=true`) |
...
If fewer than 2 positional tokens are provided, print: `**ERROR: Usage: /alias [--merge] [--no-slack] <alias-name> <target-skill> [preset-flags...]**` and abort.
...
Print: `**Alias /<alias-name> -> /<target-skill> <preset-flags> — delegating to /implement --quick --auto [--merge] [--no-slack]**` (omit `<preset-flags>` if empty; omit `--merge` if `alias_merge=false`; omit `--no-slack` if `alias_no_slack=false`).
