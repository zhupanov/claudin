...
- `scripts/deny-edit-write.sh` is the skill-scoped PreToolUse deny hook registered in `skills/research/SKILL.md`'s `hooks:` frontmatter — it denies `Edit|Write|NotebookEdit` while `/research` is active by emitting a fixed `hookSpecificOutput` JSON deny envelope via `jq -cn`. The script ignores stdin (the matcher already restricts the trigger), is composed only of fixed ASCII literals (no runtime interpolation into the deny JSON), and uses `set -uo pipefail` with no trailing `exit 0` so `jq`'s exit status propagates on failure. This is a defense-in-depth second layer; the primary mechanical control remains the `allowed-tools` frontmatter that omits `Edit`/`Write`/`Skill` (see `SECURITY.md`). `scripts/test-deny-edit-write.sh` is its regression harness, wired into `make lint` via the `test-deny-edit-write` target; edits to the hook must stay in sync with the harness. The test script is added to `agent-lint.toml`'s exclude list because agent-lint's dead-script rule does not follow Makefile-only references.
