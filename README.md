...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><code>[--debug] [--plan] [--interactive] [--scale=standard|deep] [--quick] [--adjudicate] [--keep-sidecar[=PATH]] [--token-budget=N] &lt;research question or topic&gt;</code></td>
    </tr>
    <tr><td colspan="2">Collaborative <strong>best-effort</strong> read-only research with two orthogonal mode selectors: <code>--scale=standard|deep</code> (default <code>standard</code>) and <code>--quick</code> (boolean, default <code>false</code>; mutually exclusive with explicit <code>--scale=standard|deep</code>). <code>--scale=standard</code> (default) runs 3 research agents (Cursor, Codex, Claude inline) <strong>angle-differentiated per lane</strong> (architecture / edge cases or external comparisons / security) + 3-reviewer validation panel; <code>--quick</code> runs K=3 homogeneous Claude inline lanes carrying identical baseline prompts and structured-output rubrics, vote-merged at synthesis via reproducible Jaccard-based clustering — Step 2 is skipped (fastest path; K-lane vote-merge confidence — lower assurance; correlated-error risk remains because all 3 lanes are Claude); three-tier partial-failure handling (3/3 / 2/3 / &lt;2 fail-closed). <code>--scale=deep</code> runs 5 research lanes with the four diversified angle prompts (architecture / edge cases / external comparisons / security) plus a baseline Claude inline lane + 5-reviewer validation panel. Optional <code>--plan</code> (supported with <code>--scale=standard</code> or <code>--scale=deep</code>; downgraded with a warning under <code>--quick</code>, since K=3 vote-merge requires homogeneous lanes) decomposes the question into 2–4 focused subquestions via a planner pre-pass, assigns them to the per-scale lanes (3 standard — each subquestion inherits its lane's angle perspective; 5 deep — balanced partial-matrix ring rotation across the 4 named-angle slots plus a Claude-inline integrator), and organizes synthesis by subquestion (deep mode also adds a Per-angle highlights sub-section + Cross-cutting findings sub-section); falls back cleanly to single-question mode on planner failure. Optional <code>--interactive</code> (requires <code>--plan</code>; hard-fails on non-TTY before the planner runs) pauses after the planner so the operator can review, edit (via <code>$EDITOR</code> or stdin fallback), or abort the proposed subquestions before lane fan-out. Optional <code>--keep-sidecar</code> (or <code>--keep-sidecar=&lt;PATH&gt;</code>) preserves a <code>/issue</code>-batch markdown sidecar of findings (one <code>### &lt;title&gt;</code> block per finding, parseable by <code>/issue --input-file</code>) past the Step 4 cleanup; under <code>--quick</code>, the orchestrator-internal <code>quick-ballot.md</code> (per-lane parsed claims + cluster mapping) is also preserved alongside the findings sidecar (using the same destination-validation guardrails) — defaults to <code>./research-findings-batch.md</code> + <code>./research-findings-batch-ballot.md</code> in the operator's working directory; opt-in workspace write via Bash <code>cp</code> (see <a href="SECURITY.md#external-reviewer-write-surface-in-research-and-loop-review">SECURITY.md</a>). Mechanically guarded only on the Claude <code>Edit</code>/<code>Write</code>/<code>NotebookEdit</code> surface; Bash and external reviewers are prompt-enforced (see <a href="skills/research/SKILL.md">SKILL.md</a> &amp; <a href="SECURITY.md#external-reviewer-write-surface-in-research-and-loop-review">SECURITY.md</a>).</td></tr>
    <tr><td colspan="2"><hr></td></tr>
    <tr>
