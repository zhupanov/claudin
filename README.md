...
<table>
  ...
  <tbody>
    ...
    <tr><td colspan="2">Collaborative <strong>best-effort</strong> read-only research, scale-aware. <strong>Adaptive scaling is the default</strong>: a deterministic shell classifier inspects the question at Step 0.5 and picks <code>quick|standard|deep</code> automatically; <code>--scale=...</code> is a manual override; on classifier failure <code>RESEARCH_SCALE</code> falls back to <code>standard</code>. <strong>Step 2.7 (citation validation)</strong> runs unconditionally on every scale: a deterministic shell validator extracts cited URLs / DOIs / file:line references from the synthesis, HEAD-fetches URLs under SSRF guards, validates DOIs syntactically + via <code>doi.org</code> HEAD, spot-checks file:line ranges against the git tree, and writes a 3-state ledger (PASS / FAIL / UNKNOWN) sidecar that Step 3 splices into the final report — fail-soft (per-claim failures surface as warnings; the report is never blocked). Standard scale runs 3 research agents (Cursor, Codex, Claude inline) <strong>angle-differentiated per lane</strong> (architecture / edge cases or external comparisons / security) + 3-reviewer validation panel; <code>--scale=quick</code> runs <strong>K=3 homogeneous Claude Agent-tool lanes with vote-merge synthesis</strong> (issue #520) and skips Step 2 (the validation panel) but still renders a 0-reviewer Validation phase placeholder line in the final report (fastest, K-lane voting confidence — correlated-error risk: same model + same prompt; voting catches independent stochastic errors only); <code>--scale=deep</code> runs 5 research lanes with the four diversified angle prompts (architecture / edge cases / external comparisons / security) plus a baseline Claude inline lane + 5-reviewer validation panel. Optional <code>--plan</code> (supported with <code>--scale=standard</code> or <code>--scale=deep</code>; downgraded with a warning under <code>--scale=quick</code>) decomposes the question into 2–4 focused subquestions via a planner pre-pass, assigns them to the per-scale lanes (3 standard — each subquestion inherits its lane's angle perspective; 5 deep — balanced partial-matrix ring rotation across the 4 named-angle slots plus a Claude-inline integrator), and organizes synthesis by subquestion (deep mode also adds a Per-angle highlights sub-section + Cross-cutting findings sub-section); falls back cleanly to single-question mode on planner failure. Optional <code>--interactive</code> (requires <code>--plan</code>; hard-fails on non-TTY before the planner runs) pauses after the planner so the operator can review, edit (via <code>$EDITOR</code> or stdin fallback), or abort the proposed subquestions before lane fan-out. Optional <code>--keep-sidecar</code> (or <code>--keep-sidecar=&lt;PATH&gt;</code>) preserves a <code>/issue</code>-batch markdown sidecar of findings (one <code>### &lt;title&gt;</code> block per finding, parseable by <code>/issue --input-file</code>) past the Step 4 cleanup — defaults to <code>./research-findings-batch.md</code> in the operator's working directory; opt-in workspace write via Bash <code>cp</code> (see <a href="SECURITY.md#external-reviewer-write-surface-in-research-and-loop-review">SECURITY.md</a>). Mechanically guarded only on the Claude <code>Edit</code>/<code>Write</code>/<code>NotebookEdit</code> surface; Bash and external reviewers are prompt-enforced (see <a href="skills/research/SKILL.md">SKILL.md</a> &amp; <a href="SECURITY.md#external-reviewer-write-surface-in-research-and-loop-review">SECURITY.md</a>).</td></tr>
    <tr><td colspan="2"><hr></td></tr>
    <tr>
    ...
    <tr><td colspan="2">Evolve an existing larch skill: validate and locate the target, run <code>/research --scale=deep</code> against repo-local sibling skills + reputable external sources (Anthropic / OpenAI / DeepMind / ≥500-star OSS) for concrete actionable improvements with citations, then if findings emerge delegate to <code>/umbrella</code>. <code>/umbrella</code>'s own classifier decides one-shot vs multi-piece on the distilled task description: multi-piece yields an umbrella tracking issue plus one child per piece (decomposition pieces, not necessarily 1:1 with research findings); one-shot yields a single issue (no umbrella). Tagged <code>evolved-by:skill-evolver</code> + <code>skill:&lt;name&gt;</code> in either path. Research-and-file-issues only — does not modify the target skill's files; implementation lands later via <code>/fix-issue</code>, <code>/improve-skill</code>, or <code>/loop-improve-skill</code>.</td></tr>
  </tbody>
</table>
