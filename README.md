...
<table>
  ...
  <tbody>
    ...
    <tr><td colspan="2">Collaborative <strong>best-effort</strong> read-only research, scale-aware: <code>--scale=standard</code> (default) runs 3 research agents + 3-reviewer validation panel (byte-equivalent to pre-#418 behavior); <code>--scale=quick</code> runs 1 inline Claude lane and skips validation entirely (fastest, single-lane confidence — lower assurance); <code>--scale=deep</code> runs 5 research lanes with diversified angle prompts (architecture / edge cases / external comparisons / security) + 5-reviewer validation panel. Optional <code>--plan</code> (standard scale only) decomposes the question into 2–4 focused subquestions via a planner pre-pass, assigns them to the 3 lanes, and organizes synthesis by subquestion; falls back cleanly to single-question mode on planner failure. Mechanically guarded only on the Claude <code>Edit</code>/<code>Write</code>/<code>NotebookEdit</code> surface; Bash and external reviewers are prompt-enforced (see <a href="skills/research/SKILL.md">SKILL.md</a> &amp; <a href="SECURITY.md#external-reviewer-write-surface-in-research-and-loop-review">SECURITY.md</a>).</td></tr>
    <tr><td colspan="2"><hr></td></tr>
    <tr>
